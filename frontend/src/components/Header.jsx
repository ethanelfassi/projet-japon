...
import { Camera, MapPin, Compass, Users, User, LogOut, Shield, CalendarDays } from 'lucide-react';

const Header = ({ activeTab, setActiveTab, user, onLogout }) => {
  return (
    <header className="glass" style={{
      position: 'fixed', top: '15px', left: '50%', transform: 'translateX(-50%)',
      width: '95%', maxWidth: '600px', zIndex: 1000, padding: '8px 15px',
      ...
      {user ? (
        <>
          ...
          <NavButton active={false} onClick={onLogout} icon={<LogOut size={20} />} label="Quitter" />
        </>
      ) : (
        <NavButton active={activeTab === 'auth'} onClick={() => setActiveTab('auth')} icon={<User size={20} />} label="Connexion" />
      )}
    </header>
  );
};
...
const NavButton = ({ active, onClick, icon, label }) => (
  <button
    ...
    style={{
      ...
      gap: '4px', padding: '4px 8px', transition: 'all 0.3s ease',
